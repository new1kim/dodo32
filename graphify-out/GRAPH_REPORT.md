...
## Corpus Check
- 1952 files · ~132,305 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 238 nodes · 469 edges · 19 communities (16 shown, 3 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 22 edges (avg confidence: 0.55)
...
Cohesion: 0.15
Nodes (24): _0x296e(), _0xd5a11, _0xdf61(), applyRatioResultStyle(), buildScheduleRow(), calculateLtvMaxAmount(), formatKoreanAmount(), generateSchedule() (+16 more)
...
## Knowledge Gaps
- **74 isolated node(s):** `_0xd5a11`, `DEFAULT_FIRST_ROW_FIELD_IDS`, `APT_PRICE_FIELD_LABELS`, `DEFAULT_LOAN_RATE_TABLE`, `savedRates` (+69 more)
...
- **Why does `자동계산()` connect `DSR_calculator_logic.min.js` to `loadDSRInputs`?**
  _High betweenness centrality (0.084) - this node is a cross-community bridge._
- **Why does `setupNeedDatePicker()` connect `setupNeedDatePicker` to `DSR_calculator_ui.js`, `showBubble`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `adjustTableFontSize()` connect `DSR_calculator_logic.min.js` to `DSR_calculator_ui.js`, `showBubble`, `loadDSRInputs`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Are the 13 inferred relationships involving `자동계산()` (e.g. with `calculateLtvMaxAmount()` and `formatKoreanAmount()`) actually correct?**
  _`자동계산()` has 13 INFERRED edges - model-reasoned connections that need verification._
- **What connects `_0xd5a11`, `DEFAULT_FIRST_ROW_FIELD_IDS`, `APT_PRICE_FIELD_LABELS` to the rest of the system?**
