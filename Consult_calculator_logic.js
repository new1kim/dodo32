...
function applyRatioResultStyle(el, ratioPercent) {
  ...
  if (ratioPercent >= 40) {
    el.style.setProperty("background-color", "#fbeceb", "important");
    el.style.color = "#c2453d";
  } else {
    el.style.setProperty("background-color", "transparent", "important");
    el.style.color = "#151b26";
  }
}
...
function resetDsrMaxBlocks() {
  ['DSR최대금액확인-원리금', 'DSR최대금액확인-원금'].forEach(id => {
    ...
    if (el) {
      ...
      const subEl = el.querySelector('.dsr-sub-val');
...
function 자동계산() {
  // 소득1(computedIncomeHidden) + "소득 추가"로 늘어난 소득2, 소득3, ... (computedIncomeHidden_2, _3, ...)를
  // 모두 합산한다. 행 개수와 무관하게 .income-computed-hidden 클래스를 가진 hidden input을 전부 더하는 방식이라
  // 아래 계산식(DSR/DTI/신DTI) 자체는 손대지 않고, 합산 대상 소득 값 하나만 그대로 넘긴다.
  let income = [...document.querySelectorAll('.income-computed-hidden')].reduce((total, el) => {
    return total + (parseFloat((el.value || "").replace(/,/g, '')) || 0);
  }, 0);

  // 소득 행 중 1개 이상 "신고소득(카드/건강/연금)" 모드를 사용 중이면
  // 합산 소득도 최대 인정액(COMBINED_DECLARE_INCOME_CAP)을 넘지 않도록 제한한다.
  const usesDeclareIncome = (typeof baseIncomeMode !== 'undefined' && baseIncomeMode === '신고') ||
                             (typeof incomeRowState !== 'undefined' && [...incomeRowState.values()].some(st => st.mode === '신고'));
  if (usesDeclareIncome && typeof COMBINED_DECLARE_INCOME_CAP !== 'undefined') {
    income = Math.min(income, COMBINED_DECLARE_INCOME_CAP);
  }

  const totalIncomeOutput = document.getElementById("totalIncomeOutput");
  if (totalIncomeOutput) {
    const totalIncomeStr = income > 0 ? Math.floor(income).toLocaleString() : "";
    totalIncomeOutput.value = totalIncomeStr;
    // 새 "소득정보" 탭 제목줄에도 같은 합산 소득을 실시간으로 보여준다 (Consult_Main.html 쪽 UI, 계산 로직과는 무관)
    if (typeof updateIncomeInfoSummary === 'function') updateIncomeInfoSummary();
    // size 속성은 "0" 기준 평균폭으로 계산되어 콤마가 섞인 숫자에는 여백이 크게 남는다.
    // 폰트 크기를 고정하고 실제 렌더링 폭(canvas measureText)에 맞춰 픽셀 단위로 꽉 차게 지정한다.
    // 여유폭을 fitNumericInputFontSize류의 전역 자동축소 스크립트가 쓰는 여유(clientWidth - 6)와
    // 맞춰줘야, 그 스크립트가 다시 실행돼도 불필요하게 글자를 줄이지 않는다 (계산 로직과는 무관한 표시 전용 처리)
    const FONT_PX = 16;
    totalIncomeOutput.style.fontSize = FONT_PX + 'px';
    totalIncomeOutput.dataset.origSize = FONT_PX + 'px'; // 전역 자동축소 스크립트가 이 값을 기준폭으로 쓰게 함
    const cs = getComputedStyle(totalIncomeOutput);
    const ctx = document.createElement('canvas').getContext('2d');
    ctx.font = `${cs.fontWeight} ${FONT_PX}px ${cs.fontFamily}`;
    const displayText = totalIncomeStr || totalIncomeOutput.placeholder || '';
    const textWidth = ctx.measureText(displayText).width;
    totalIncomeOutput.style.width = Math.ceil(textWidth + 8) + 'px';
  }
    ...
    if (firstRow) {
      ...
      if (combinedRate1 > 0 && term1 > 0) {
        ...
        const setDsrBlockValues = (elId, rawLoanVal) => {
          ...
          const formattedIncomeMax = formatPlainWon(Math.round(rawLoanVal));
          if (ltvCapAmount !== Infinity && ltvCapAmount <= 0) {
            ...
            const formattedLtv = formatPlainWon(Math.round(ltvCapAmount));
        ...
        if (availableForMortgage <= 0) {
          if (rw) { rw.querySelector('.dsr-main-val').innerText = "대출 불가"; const rwSub = rw.querySelector('.dsr-sub-val'); if (rwSub) rwSub.innerText = ""; }
          if (rg) { rg.querySelector('.dsr-main-val').innerText = "대출 불가"; const rgSub = rg.querySelector('.dsr-sub-val'); if (rgSub) rgSub.innerText = ""; }
        } else {
  ...
  // 새 "DSR정보"/"대출정보" 탭 제목줄에도 같은 DSR/DTI/신DTI 값을 실시간으로 보여준다
  // (Consult_Main.html 쪽 UI, 계산 로직과는 무관)
  ...
  updateLtvMaxAmount();
  if (typeof updateMortgageReductionButtons === 'function') updateMortgageReductionButtons();
  
  setTimeout(() => {
    if (typeof refreshDsrMaxFontSizeStably === 'function') refreshDsrMaxFontSizeStably();
    else if (typeof adjustDsrMaxFontSize === 'function') adjustDsrMaxFontSize();
