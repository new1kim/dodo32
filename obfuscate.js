...
// 현재 폴더(__dirname)에 있는 Consult_calculator_logic.js를 타겟으로 잡습니다.
const targetPath = path.resolve(__dirname, 'Consult_calculator_logic.js');
// 원본을 보호하기 위해 이름 끝에 .min.js를 붙여 새로운 파일로 뽑아냅니다.
const outputPath = path.resolve(__dirname, 'Consult_calculator_logic.min.js');

// 파일이 실제로 있는지 확인
if (!fs.existsSync(targetPath)) {
    console.error('❌ 원본 파일(Consult_calculator_logic.js)을 찾을 수 없습니다.');
    process.exit(1);
}
...
console.log('✨ 성공! 난독화된 Consult_calculator_logic.min.js 파일이 생성되었습니다.');
